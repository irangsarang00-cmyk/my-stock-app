...
# 1. 시트3 데이터 가져오기 (인증 로직 포함)
try:
    # 🔐 이미 파일 상단에 정의된 인증 정보(credentials)를 사용하여 구글 시트에 접속합니다.
    # 만약 기존 코드에서 gc를 다른 이름으로 쓰고 있다면, 아래처럼 새로 정의해버리는 게 가장 확실합니다.
    gc_milk = gspread.authorize(credentials) 
    milkrun_sheet = gc_milk.open_by_url(SHEET_URL).worksheet("시트3")
    
    # 데이터를 가져와서 표(DataFrame)로 만듭니다.
    milk_data = milkrun_sheet.get_all_records()
    df_milk = pd.DataFrame(milk_data)
except Exception as e:
    # 혹시 시트3이 없거나 연결이 안 되면 에러를 띄웁니다.
    st.error(f"밀크런 데이터를 불러오지 못했습니다. (원인: {e})")
    df_milk = pd.DataFrame()

# 메뉴를 그리는 부분 시작
st.markdown("### 📦 창고별 밀크런 현황")

# 창고 순서 정의 (2창고 -> 3창고 -> 1창고)
target_warehouses = [
...
if not df_milk.empty:
    for wh in target_warehouses:
        # 해당 창고 데이터만 필터링 (F열 헤더가 '창고'인 경우)
        # 만약 시트의 헤더 이름이 '창고'가 아니라면 아래 ['창고']를 실제 이름으로 바꿔주세요.
        ...
        with st.expander(wh['name'], expanded=False):
            if filtered_df.empty:
                ...
                for _, row in filtered_df.iterrows():
                    # 버튼 제목: [시간] 차량번호
                    expander_title = f"🚚 [{row.get('도착예상시간', '시간미정')}] {row.get('차량No', '번호미상')}"
                    
                    with st.expander(expander_title):
                        # 5개 열로 나누어 정보 표시
                        m1, m2, m3, m4, m5 = st.columns(5)
                        m1.caption("차량")
                        m1.write(row.get('차량No', '-'))
                        m2.caption("시간")
                        m2.write(row.get('도착예상시간', '-'))
                        m3.caption("벤더")
                        m3.write(row.get('계정', '-'))
                        m4.caption("센터")
                        m4.write(row.get('물류센터', '-'))
                        m5.caption("수량")
                        m5.write(f"{row.get('총 팔레트 수량', '0')} PL")
else:
    st.warning("시트에서 데이터를 읽어오지 못했습니다.")
